      ...
      <div className="pointer-events-auto inline-flex rounded-full border border-white/60 bg-white/70 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.15)]">
        <nav className="flex items-center justify-center gap-3 px-3 py-2" aria-label="CHW Navigation">
          {tabs.map((tab) => {
            ...
            const isPrimary = tab.id === "reports"; // emphasize central action
            const baseBtn =
            ...
            const primaryBtn = "w-12 h-12 bg-[#00ff88] text-black shadow-[0_4px_16px_rgba(0,255,136,0.25)]";
            const defaultBtn = `${
              isActive ? "bg-white/90 text-slate-900" : "bg-white/60 text-slate-700 hover:bg-white/75"
